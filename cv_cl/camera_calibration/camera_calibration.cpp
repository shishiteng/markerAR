...
public:
    ...
    void write(FileStorage& fs) const//写初始化数据到文件//Write serialization for this class，
     {
        fs << "{" << "BoardSize_Width"  << boardSize.width//图像宽度
                  ...
                  << "Calibrate_AssumeZeroTangentialDistortion" << calibZeroTangentDist//切向畸变参数是否设为0,就是不考虑。
    ...
    void interprate()//对读入的参数进行正确性判断，并赋值给某些变量
      {
        goodInput = true;
    ...
    static bool readStringList( const string& filename, vector<string>& l )//把数据导入到字符向量中。
    {
        l.clear();
...
static void read(const FileNode& node, Settings& x, const Settings& default_value = Settings())//定义读函数。
 {
    if(node.empty())
        x = default_value;
    else
...
int main(int argc, char* argv[])
 {
    ...
    fs["Settings"] >> s;
    fs.release();  //复制这个设置文件，然后释放fs。// close Settings file

    if (!s.goodInput)//如果给出的初始化参数没有错误，就继续，有错误就返回。
     {
    ...
    clock_t prevTimestamp = 0;//用于计时
    const Scalar RED(0,0,255), GREEN(0,255,0);//定义颜色信息
    ...
    for(int i = 0;;++i)
     {
      ...
      view = s.nextImage();//每次读出一帧图像

      //-----  If no more image, or got enough, then stop calibration and show result -------------
      //如果没有更多的图像或已有足够的图像，就停止校正，展示结果。
...
static void calcBoardCornerPositions(Size boardSize, float squareSize, vector<Point3f>& corners,
                                     Settings::Pattern patternType /*= Settings::CHESSBOARD*/)//计算棋盘格特征点的三维位置，z为0
{
    corners.clear();
